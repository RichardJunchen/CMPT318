...
data_project <- data_project[data_project$datetime$year == 108 &
                               data_project$datetime$mday == 12,]

ggplot() + 
  layer(data=data_project, mapping=aes(x=Time, y=Global_active_power), stat="identity", geom = "point", position = "identity") + 
  xlab("Time") + 
...
plot(Nstates, BIC1, ty="b",main = 'Single model')
# nstates = 12
...
plot(Nstates, BIC2, ty="b",main = "multiple model")
# nstates = 12 
...
plot(Nstates, BIC3, ty="b",main = 'PCA single model')
# nstates = 12
...
plot(Nstates, BIC4, ty="b",main = 'PCA multiple model')
# nstates = 12
